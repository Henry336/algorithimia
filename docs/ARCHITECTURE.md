...
`algorithimia.game_shell` is the first browser game-shell surface. It exports a self-contained static browser screen for the current encounter roster through `--game-html`, showing generated build context in the normal shell header and opening on a real Algorithimia title/home screen. The companion `--static-bundle` command writes the same shell as `index.html` in a deployable folder with a minimal `vercel.json` and local/deploy notes, so the current title/home plus Chapter 0 shell can be checked locally or handed to Vercel as a static artifact without adding a backend, database, environment variables, or public code-execution service. The title screen is the current static-bundle front door: `Start Chapter 0` reveals a compact Chapter 0 briefing where Mira frames the jammed intake, Sorting Slime, visible rune repair, and non-disclosing Archive trust check; `Begin repair` reveals a lightweight intake-approach handoff; `Enter intake` then reveals the existing tiny explorable Queueworks room before the encounter tabs. The handoff is a DOM screen-state transition using existing room/gate sprites, not a new map, encounter, renderer, or story expansion. The room opens with Mira's arrival line and a one-line objective to find the Sorting Slime and inspect the loose runes. The room uses simple DOM-positioned sprites, arrow/WASD/on-screen movement, lightweight blocked-tile collision for the Sorting Slime, jammed gate, and ledger clutter, an interactable Sorting Slime object, Agent 5's package-local Queueworks room sheet for the temporary Patchrunner and interaction prompt, Agent 5's room feedback sheet for movement/interact/failure/sealed/open status, Agent 5's room retry strip for the failed-return prompt, wrong-order return-to-room retry feedback, and a success return path that clears the jammed route blockers after the visible repair succeeds. The route-clear return now leaves a compact aftermath line that the stair accepts the repair and the intake starts moving again. The room status strip binds the current Agent 1/2 affordance cues to live states: blocked gate, visible-order retry, sealed-check retry, and route clear. Once the route is clear, repeated Sorting Slime interaction is disabled and the room cue stays on route-open feedback. The Sorting Slime panel includes a local browser interaction loop with rune-token selection, swaps, icon-supported check/reset controls, adjacent inspection marks, jammed/cleared route status, Mira feedback, and embedded Phase 1 Sorting Slime scene strip, Queueworks gate, Sorting Slime sprites, and action icons. Opening the exported shell with `?smoke=1` or `#smoke` runs a dependency-free in-page smoke path that verifies the title screen, start control, Chapter 0 briefing, begin-repair control, room handoff, and enter-intake control, enters the room, checks the arrival objective, then records whether the room viewport shows the normal build context, room, room status, room hint, and Sorting Slime target marker before any report-scrolling checks. It then covers keyboard/WASD movement via dispatched `keydown` events, on-screen movement, interaction, wrong-order return, rune swaps, success return, route-clear aftermath, repaired-state interaction, route-open status, visible cue dimensions, horizontal-overflow checks, 40px control-target checks, text-fit checks for key status cues, smoke-report row readability checks, and copyable build-context evidence; its smoke rows use Agent 5's `queueworks-browser-smoke-icons.svg` QA/status sheet while the normal playtest screen is unchanged. The smoke report also renders the shared browser evidence intake card fields so a live pass or fail can be copied back with generated build context, state tested, result, control path, viewport, viewport profile, orientation, page size, first viewport, capture quality, manual review prompt, observed cause, text/icon/collision agreement, likely owner, and next action. The report exposes the same content through a read-only copy field and `data-smoke-result`, `data-smoke-label`, `data-smoke-viewport`, `data-smoke-viewport-profile`, `data-smoke-orientation`, `data-smoke-page-size`, `data-smoke-first-viewport`, and `data-smoke-build` attributes for browser QA tooling. This interaction is a graphical vertical-slice bridge; it intentionally does not run player Python in the browser. Execution remains in the existing CLI adapter until the project has a stronger sandbox and a chosen live renderer target.
