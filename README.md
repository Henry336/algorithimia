...
Export the current static browser bundle for local checks or Vercel-style static hosting:

```powershell
python -m algorithimia --static-bundle build/site
```

Open `build/site/index.html` in a browser, or deploy the generated `build/site/` folder as a static site. The bundle includes `index.html`, a minimal `vercel.json`, and local smoke/deploy notes. It does not need a backend, database, environment variables, or public code-execution service.

Open `build/game-shell.html` in a browser. The exported static bundle now opens on the Algorithimia title/home screen; choose `Start Chapter 0` to read the compact Chapter 0 briefing, choose `Begin repair` to see the intake approach handoff, then choose `Enter intake` to enter the current playable Queueworks room. The room opens with Mira's arrival line and a clear objective to find the Sorting Slime and inspect the loose runes. The header shows the generated build context so Henry or Agent 4 can spot stale exported HTML during normal play. In Chapter 0, move the Patchrunner with arrow keys, WASD, or the on-screen buttons, navigate around blocked ledger/gate tiles, interact with the Sorting Slime, sort the visible rune spill, check the order, then return to the room. A correct order opens the route, clears the jammed gate blockers, shows the intake moving again, and disables repeated repair interaction; a wrong order returns to the same room with retry feedback and the intake still jammed. The room status strip now names blocked-gate, retry, sealed-check, and route-clear states directly.

For a lightweight browser self-smoke, open the same file with `?smoke=1` or `#smoke` appended. The page verifies the title screen, `Start Chapter 0` control, Chapter 0 briefing, `Begin repair` control, intake handoff, and `Enter intake` control first, then enters Chapter 0, checks the arrival objective, captures first-viewport room composition including the Sorting Slime target marker, and exercises keyboard/WASD movement, on-screen movement, interaction, wrong-order return, rune swaps, check order, success return, route-clear aftermath, repaired-state interaction, route-open status, rendered cue readability, control target sizes, text-fit checks, and horizontal-overflow checks. It then prints an icon-backed pass/fail report inside the room panel. The report also includes a browser evidence intake card with the generated build context, tested states, result, control path, viewport, viewport profile, orientation, page size, first-viewport summary, capture-quality prompt, manual-review prompt, likely owner, and next action, plus a read-only copy field and `data-smoke-*` metadata so Agent 4 or Henry can capture the exact live-browser finding for the current browser gate. The smoke path verifies that the copyable evidence includes the current build context and exposes orientation as `data-smoke-orientation`.
...
The exported shell also has a dependency-free `?smoke=1` / `#smoke` self-check for Agent 4 and Henry. It uses Agent 5's browser-smoke QA/status icon sheet only inside that smoke report and stays inactive during normal play. The self-check records whether the initial viewport shows the normal header build context, room, status, hint, and Sorting Slime target marker before later smoke checks scroll the report into view. It then dispatches real browser `keydown` events for Arrow/WASD movement, uses the on-screen controls for the rest of the route, verifies that key room/encounter cues are rendered with visible dimensions in the active browser viewport, checks that key controls keep at least 40px tap targets, checks important cue text for clipping, checks the page for horizontal overflow at each state, confirms the repaired interaction stays route-open, and verifies the copyable evidence includes the generated build context before reporting pass. The report includes an evidence card that mirrors the shared live-browser intake format: generated build context, state tested, pass/fail label, control path, viewport, viewport profile, orientation, page size, first viewport, capture quality, manual review prompt, observed cause, text/icon/collision agreement, likely owner, and next action. The same evidence is exposed through a read-only copy field and `data-smoke-result`, `data-smoke-label`, `data-smoke-viewport`, `data-smoke-viewport-profile`, `data-smoke-orientation`, `data-smoke-page-size`, `data-smoke-first-viewport`, and `data-smoke-build` attributes for easier browser QA capture.
