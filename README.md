...
For a lightweight browser self-smoke, open the same file with `?smoke=1` or `#smoke` appended. The page will exercise keyboard/WASD movement, on-screen movement, interaction, wrong-order return, rune swaps, check order, success return, repaired-state interaction, route-open status, rendered cue readability, control target sizes, text-fit checks, and horizontal-overflow checks, then print an icon-backed pass/fail report inside the room panel. The report also includes a browser evidence intake card with the tested states, result, control path, viewport, likely owner, and next action so Agent 4 or Henry can copy the exact live-browser finding into the DEC-008 gate.
